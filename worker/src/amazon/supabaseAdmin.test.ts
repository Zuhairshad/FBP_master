...
describe('getAmazonTokenForBrand', () => {
  it('returns the row when one exists', async () => {
    ...
    const row = await getAmazonTokenForBrand(env, 'brand-1', fetchImpl)
  ...
  it('returns null when no row exists (maybeSingle, PostgREST 406-style empty)', async () => {
    ...
    const row = await getAmazonTokenForBrand(env, 'brand-2', fetchImpl)
    expect(row).toBeNull()
  })
  ...
  it('throws with a descriptive message on a database error', async () => {
    ...
    await expect(getAmazonTokenForBrand(env, 'brand-1', fetchImpl)).rejects.toThrow(/Failed to load amazon_tokens/)
  })
})

describe('upsertAmazonRefreshToken', () => {
  it('posts the refresh token row with an on-conflict upsert', async () => {
    ...
    await upsertAmazonRefreshToken(
      env,
      { brandId: 'brand-1', marketplaceId: 'ATVPDKIKX0DER', refreshToken: 'Atzr|x' },
      fetchImpl,
    )
  })
  ...
  it('throws on a database error', async () => {
    ...
    await expect(
      upsertAmazonRefreshToken(env, { brandId: 'brand-1', marketplaceId: 'x', refreshToken: 'y' }, fetchImpl),
    ).rejects.toThrow(/Failed to upsert amazon_tokens/)
  })
})
...
describe('cacheAccessToken', () => {
  it('patches the row with the new access token + expiry', async () => {
    ...
    await cacheAccessToken(
      env,
      { brandId: 'brand-1', accessToken: 'Atza|x', accessTokenExpiresAt: '2026-01-01T00:00:00Z' },
      fetchImpl,
    )
  })
})
  ...
  it('posts the order with an on-conflict upsert on (platform, platform_order_id)', async () => {
    ...
    await upsertPlatformOrder(
      env,
      {
        ...
        raw_data: { AmazonOrderId: '111-1111111-1111111' },
